/* =====================================================
   ...
   Complete POS with Camera Barcode Scanner
   Connected with: Customer Store via localStorage
   ===================================================== */

/* ============ STORAGE KEYS (Shared with Store) ============ */
const LS_KEYS = {
...
function showPOS() {
  ...
  renderRecentOrders();
  navigateTo('home');
  setTimeout(startFullscreenTimer, 8000);
}
...
function savePickedBox() {
  ...
  showToast(`${boxBuilderState.boxType.name} with ${boxBuilderState.boxType.items?.length || boxBuilderState.items.length} products added`, 'bx-box');
  document.getElementById('pickBoxModal').classList.remove('active');
}
...
function renderInventoryHistory() {
  ...
  if (!items.length) {
    list.innerHTML = `<div class="inv-empty-small"><i class='bx bx-history'></i><p>No movements yet</p></div>`;
    return;
  }
...
/* ============ END ============ */
console.log('%c🍬 Hat Candy POS Ready', 'color:#e2015d;font-weight:bold;font-size:14px;');